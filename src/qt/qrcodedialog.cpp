...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DEC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("deciphercoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DEC unit input in lnReqAmount, we generate the URI with DEC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DEC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
