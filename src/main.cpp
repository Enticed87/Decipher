...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uDEC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/87.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
void ThreadScriptCheck() {
    RenameThread("deciphercoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1397636595, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x12d73200 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x12, 0xd7, 0x32, 0x00 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeciphercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DeciphercoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("DeciphercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("deciphercoin-miner");
    ...
    try { loop {
        ...
        printf("Running DeciphercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DeciphercoinMiner terminated\n");
        throw;
    }
}
